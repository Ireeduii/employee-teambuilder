...
import { useClerk } from "@clerk/nextjs"; // 1. Clerk-ийн hook-ийг импортлох
import {
  LayoutDashboard,
  Users,
  Layers,
...
export const Sidebar = () => {
  const pathname = usePathname();
  const { signOut } = useClerk(); // 2. signOut функсийг гаргаж авах
  const router = useRouter();

  const handleSignOut = async () => {
    // 3. Гарах үйлдэл хийгээд нэвтрэх хуудас руу чиглүүлэх
  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 dark:text-slate-100 flex flex-col justify-between h-screen sticky top-0">
      ...
      <div className="p-4 border-t border-slate-100 dark:border-slate-800">
        {/* 4. onClick дээр handleSignOut функцээ холбох */}
        <button
          onClick={handleSignOut}
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all"
        >
          <LogOut className="w-4 h-4" />
          <span>Log Out</span>
        </button>
