...
export default function TeamBuilderPage() {
  const router = useRouter(); // 2. useRouter инстанц үүсгэх
  const [members, setMembers] = useState<TeamMember[]>([]);
  const [selectedSkills, setSelectedSkills] = useState<string[]>([]);
  const [skillInput, setSkillInput] = useState("");
  const [selectedMembers, setSelectedMembers] = useState<TeamMember[]>([]);
  const [teamName, setTeamName] = useState("");
  const [loading, setLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false); // Хадгалж байх үед loader харуулах state
  ...
  const handleSaveTeam = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Амжилттай хадгалагдвал шууд Багийн гишүүд хуудас руу шилжинэ
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm h-fit space-y-5">
              ...
              <button
                ...
                className="w-full py-2.5 bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-600 hover:to-teal-600 text-white rounded-xl font-medium text-sm disabled:opacity-40 disabled:cursor-not-allowed shadow-sm shadow-cyan-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>
